    ...
    <div className="w-full flex flex-col lg:flex-row gap-[15px] md:gap-[20px] lg:gap-[25px]">
      ...
      <div className="flex-4 flex flex-col gap-[32px] md:gap-[40px] lg:gap-[60px] pb-[20px] md:pb-0">
        <div className="flex flex-col gap-[20px] md:gap-[24px] lg:gap-[32px]">
          <h2
            className="text-[28px] sm:text-[32px] md:text-[40px] lg:text-[40px] font-semibold leading-[0.85] text-[var(--white)]"
          >
            {t("leftColumnTitle")}
          </h2>
          <div
            className="text-[16px] sm:text-[18px] md:text-[20px] lg:text-[22px] sm:leading-[26px] md:leading-[30px] lg:leading-[32px] text-[var(--white)] space-y-[15px]"
          >
            {descriptionParagraphs.map((paragraph, index) => (
              <p className="leading-[1]" key={index}>{paragraph}</p>
            ))}
          </div>
        </div>
      </div>
      ...
      >
        <h3 className="text-[26px] sm:text-[30px] md:text-[34px] lg:text-[40px] font-semibold leading-[0.85] text-[var(--white)]">
          {t("education.title")}
        </h3>
        <div className="space-y-[30px] sm:space-y-[20px] md:space-y-[30px]">
          {educationItems.map((item: EducationItem, index: number) => (
            ...
            >
              <div className="flex justify-between items-center text-[13px] sm:text-[14px] md:text-[15px] gap-[8px]">
                <span
                  className="px-[12px] sm:px-[15px] py-[5px] text-[16px] rounded-[15px] bg-[var(--gray-3)] text-[var(--white)]"
                >
                  {item.period}
                </span>
              ...
              <div className="text-[13px] sm:text-[14px] md:text-[14px] leading-[1.1]">
                {item.institution}
              </div>
              <div className="font-semibold text-[16px] sm:text-[17px] leading-[1]">
                {item.specialization}
              </div>
            </div>
          ))}
      ...
      <div className="flex-3 flex flex-col gap-[32px] md:gap-[40px] pt-[20px] md:pt-0">
        <div>
          <h3
            className="text-[26px] sm:text-[30px] md:text-[34px] lg:text-[40px] font-semibold mb-[20px] md:mb-[24px] leading-[0.85] text-[var(--white)]"
          >
            {t("experience.title")}
          </h3>
          <div className="space-y-[16px] md:space-y-[20px]">
            <div className="w-full">
              <div className="flex justify-between items-center mb-[12px] md:mb-[16px] w-full gap-[10px]">
                ...
                <div className="border-l-[1px] border-b-[1px] border-[var(--white)] px-[8px] md:px-[10px] font-semibold text-center text-[12px] sm:text-[13px] md:text-[18px]">
                  {t("experience.active")}
                </div>
              </div>
              <div className="text-[18px] sm:text-[19px] md:text-[20px] font-bold mb-[20px] text-[var(--white)] flex flex-col">
              ...
              <ul className="space-y-[6px] md:space-y-[10px] text-[16px] sm:text-[18px] md:text-[20px] text-[var(--white)] list-disc pl-[20px]">
                {t.raw("experience.current.description").map(
                  (item: string, index: number) => (
                    <li className="leading-[1]" key={index}>{item}</li>
                  ),
                )}
              </ul>
            </div>
